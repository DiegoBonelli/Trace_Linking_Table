####
####
#  R Code to create linking table from Academic TRACE to CRSP
#   
...
# Required Academic Trace in Data/Academic Trace/
...
# list.files("Data/Temp/names")
# trace=NULL
# for (i in 2002:2021){
# x=fread(paste0("Data/Temp/names/trace_names_",i,".csv"))
# x=x[date<=mtrty_dt]
# # set order dates
# setorder(x,cusip_id,sym_cd,cmpny_nm,date)
# trace=rbind(trace,x,fill=T)
# }
# rm(x)
# 
# # # create a counter id based on cusip_id, permco and number_permco
# setorder(trace,cusip_id,date)
# trace$counter <- with(trace, rleid(cusip_id,sym_cd,cmpny_nm))
# # unify dates
# trace=unique(trace[,":="(min_date=min(date),
#                  max_date=max(date)),by=list(cusip_id,sym_cd,cmpny_nm,counter)
# ][,list(cusip_id,sym_cd,cmpny_nm,min_date,max_date,mtrty_dt)])
# save(trace,file="Data/Temp/names/trace_all.rdata")

# Get trace symbols of cusips
load("Data/Temp/names/trace_all.rdata")

# remove missing cusips
trace=trace[cusip_id!=""]
# get only standard symbols (SYMB.FIRM format)
trace$sym_cd2=sub("\\..*", "", trace$sym_cd)
# get symbols from SYMB0000000 format
trace[grepl("\\.",sym_cd)==F, sym_cd2:=substr(sym_cd,1,nchar(sym_cd)-7)]
trace=trace[,sym_cd:=sym_cd2][,-c("sym_cd2")]

# change names and unify variables
setnames(trace,"sym_cd","company_symbol")
setnames(trace,"max_date","max_trace_date")
setnames(trace,"min_date","min_trace_date")
setnames(trace,"mtrty_dt","maturity_date")
...
agent_ids=unique(fisd_issuer$parent_id) # agent_id of this issuer's parent company
fisd_issuer=fisd_issuer[agent_id%in%agent_ids] # leave only parents
fisd_issuer[,parent_id:=agent_id] # will use parent_id for merging, change the name
...
# create dates 
date.eom.bd <- unique(preceding(seq(as.Date(min(fisd1$offering_date,na.rm=T)), as.Date("2022-01-01"), by="1 day") - 1, "Rmetrics/NYSE"))
...
# remove if after 2022-01-01
fisd1=fisd1[namedt<"2022-01-01"]
...
for (i in 1:ceiling(Ncusips/10000)) {
  ...
  k=fisd2[remove==F&checked_full==T]
  # create dates 
  date.eom.bd <- unique(preceding(seq(as.Date(min(k[cusip_id%in%cusips[i:n]]$offering_date,na.rm=T)), as.Date("2022-01-01"), by="1 day") - 1, "Rmetrics/NYSE"))
...
# check if multiple permco per cusip
# create dates 
date.eom.bd <- unique(preceding(seq(as.Date(min(fisd3$offering_date,na.rm=T)), as.Date("2022-01-01"), by="1 day") - 1, "Rmetrics/NYSE"))
...
# create dates 
date.eom.bd <- unique(preceding(seq(as.Date(min(fisd4$offering_date,na.rm=T)), as.Date("2022-01-01"), by="1 day") - 1, "Rmetrics/NYSE"))
